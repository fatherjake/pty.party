...
// running on a remote host over SSH.
//
// The full mcp-server/index.js talks to the canvas through files under the
// Mac's ~/Library/Application Support/ptyparty/, which don't exist on a remote
// box. This bridge instead forwards the canvas RPC over the Unix socket that
// pty.party reverse-forwards into each remote tile (PTYPARTY_RPC), so a remote
// claude can drive the Log exactly like a local one.
//
// It needs only `node` — no npm install, no node_modules — so pty.party can
// push this single file to a host and register it with `claude mcp add`.

import { connect } from "node:net";

const TERMINAL_ID = process.env.PTYPARTY_TERMINAL_ID || null;
// PTYPARTY_RPC looks like "unix:/tmp/ptyparty-rpc-<id>.sock".
const RPC = process.env.PTYPARTY_RPC || "";
const SOCKET_PATH = RPC.startsWith("unix:") ? RPC.slice("unix:".length) : null;

// One request per connection: send a newline-framed JSON object, read one JSON
// line back. Matches the canvas broker's one-shot socket protocol.
...
// --- Tools (the subset that works over the socket) -------------------------
  ...
  switch (method) {
    case "initialize":
      send({
        ...
        id,
        result: {
          // Echo the client's protocol version when given, for forward-compat.
